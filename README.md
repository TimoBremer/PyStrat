...
The stratigraphic method is based on four rules: the laws of bedding, original horizontality, original continuity and stratigraphic sequence. In archaeology, the application of the law of stratigraphy was established in particular by E. Harris. For visualization purposes, he developed the so-called Harris matrix as seen below, in which only relationships of directly adjacent layers are shown - no indirect ones.
Smaller stratigraphic sequences can be clearly displayed with a Harris matrix, but larger complexes result in an opaque jumble of boxes and lines. The aim of stratigraphic analysis is ultimately to date the layers. The indirect relationships between the layers not recorded in a Harris matrix are of great importance for this: if, for example, layer 1 lies above layer 2 and layer 2 above layer 3, layer 1 also lies indirectly above layer 3. If layer 3 dates to 1631, it can be deduced from the analysis of the indirect relationships that layers 1 and 2 are younger than 1631. With many layers and several dates, the relationships become so complex that a “manual” analysis using Harris matrices is almost impossible. In such cases, digital analysis is required.
