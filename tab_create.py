...
def initial_db():
	...
	clean_db(['rohdaten', 'rohdaten_datierung', 'reihenf_abs_dat', 'ausschluss_rohdaten', 'vorschl_subphasen', 'ueber_unter', 'ids_gesamt', 'fehlerkette', 'dat_reihenf', 'gleich', 'ids_gruppen_gleich'])
	...
	c.execute('''CREATE VIEW IF EXISTS "dat_reihenf" AS 
	SELECT 
		dat.befund "bef",
	...
	c.execute('''CREATE VIEW IF EXISTS  abs_daten_ueber_unter_inkl_widerspr AS
		SELECT 
			befund,
	...
	c.execute('''CREATE VIEW IF EXISTS  abs_daten_ueber_unter AS
	SELECT * FROM
	...
	c.execute('''CREATE VIEW IF EXISTS  ergebnis_abs_daten AS
	SELECT
		abs_dat.befund,
	...
	c.execute('''CREATE VIEW IF EXISTS  min_max_datierungen AS
	SELECT
	...
	c.execute('''CREATE VIEW IF EXISTS  rohdaten_gef as
	select * from 
		rohdaten
	where not ROWID in
		(
	...
	c.execute('''CREATE VIEW IF EXISTS  rohdaten_geordnet AS
	select
		befund1,
		ueber_unter,
		befund2
	...
	where
		(ueber_unter = 'gleich')''')
	c.execute('''CREATE VIEW IF EXISTS  ueber_unter_befunde AS
	SELECT 
	...
	c.execute('''CREATE VIEW IF EXISTS  widerspr_strati_abs_daten AS
		WITH fehler AS
		(
		SELECT
			dat_reihenf.bef,
	...
	c.execute('''CREATE VIEW IF EXISTS fehlerk_anf AS
	SELECT
		fehlerkette1.*
	from
		(
	...
	c.execute('''CREATE VIEW IF EXISTS fehlerk_ende AS
	SELECT
		fehlerkette2.*
	from
		(
	...
	AND
		fehlerkette1.befund1 = fehlerkette2.befund2''')
	c.execute('''CREATE VIEW IF EXISTS fehlerkontrolle_1 AS
	select
		ueber_unter1.w1,
		ueber_unter1.w2
	from 
	...
	c.execute('''CREATE VIEW IF EXISTS initalf_rohdat AS
	select distinct
		roh1.*
	from 
	...
	c.execute('CREATE VIEW IF EXISTS "strat_conflicts" AS SELECT befund1, ueber_unter, befund2 FROM fehlerkette')

def create_ergeb():
	c.execute('''CREATE VIEW IF EXISTS ergebnis_strati_bef AS
	SELECT 
		id_links.befund "befund1",
		id_rechts.befund "befund2"
	FROM
